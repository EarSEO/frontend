...
import { 
  getSightDetail, 
  getSightsInRectangle} from '@/api/sight';
...
export const useSightMap = () => {
  ...
  // 영역 내 관광지 조회
  const fetchSightsInBounds = useCallback(async (bounds: RectangleBoundsParams) => {
    try {
      setLoading(true);
      setError(null);
  ...
  const fetchSightsDebounced = useCallback((bounds: RectangleBoundsParams, delay = 300) => {
  ...
  // 관광지 상세 조회
  const fetchSightDetail = useCallback(async (
    sight: SightInfo,
