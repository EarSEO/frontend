...
function MapTabScreen() {
  ...
  const {setMapMovable, setEnableCluster ,setRegionChangeCompleteMethod} = useBaseMapStore();
  ...
  useEffect(() => {
    ...
    if(onStoryAdd) {
      ...
      setMapHeaderContent(<MapSearchBar placeHolder={"관광지 검색.."} onPressMapSearchBar={() => {
        //TODO 관광지 검색 모달
      ...
      //TODO 이야기 지도용 요소 추가
      setTimeout(() => bottomSheetRef.current?.snapToIndex(1), 200);
      setMapHeaderContent(<MapSearchBar placeHolder={"이야기 검색.."} onPressMapSearchBar={() => {
        //TODO 스팟 검색 모달
  ...
  return (
    <>
      ...
      <MapHeader/>
      <CustomBottomSheet bottomSheetRef={bottomSheetRef} animatedPosition={bottomSheetPosition}/>
    </>
  );
};
